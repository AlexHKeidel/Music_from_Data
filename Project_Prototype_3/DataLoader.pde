...
class DataLoader {
  ...
  public DataLoader(String filePath) {
    ...
    integers = new int[population]; //length of the array is based on the population
    println("integers.length = " + integers.length);
    convertStringsToIntegers();
  }

  /**
   * return a specified amount of lines from the db
   **/
  public String getLines(int amount) {
    return lines[0].substring(0, amount);
  }

  /**
   * print all the lines of the file to the console
   **/
